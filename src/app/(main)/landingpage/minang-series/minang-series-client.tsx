...
interface MinangSeriesClientProps {
  minangProduct: any;
  recommendedProducts?: any[];
}

export function MinangSeriesClient({
  minangProduct,
  recommendedProducts = [],
}: MinangSeriesClientProps) {
  const [isQrisOpen, setIsQrisOpen] = useState(false);

  const demoLinks = [...(minangProduct?.product_demo_links || [])].sort(
  ...
  return (
    <>
      <section
        id="templates"
        className="py-20 md:py-24 bg-transparent border-t border-border/50"
      >
        <div className="container mx-auto px-8 md:px-6">
          <ScrollReveal className="max-w-xl mx-auto text-center mb-12 md:mb-16">
            ...
            >
              Lihat {demoLinks.length > 0 ? demoLinks.length : 5} tema premium{" "}
              <b>MINANG</b> Series
            </Typography>
          </ScrollReveal>
          ...
          {demoLinks && demoLinks.length > 0 ? (
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              {demoLinks.map((demo, index) => (
                ...
                >
                  <ProductCardDemo demoLink={demo} badgeVariant="sale" />
                </ScrollReveal>
              ))}
            </div>
          ) : (
      ...
      <section
        id="pricing"
        className="relative bg-gradient-to-br from-primary to-secondary-foreground py-20 md:py-28 overflow-hidden"
      >
        ...
        <div className="container mx-auto px-4 md:px-6 relative z-10">
          ...
          <ScrollReveal delay={200}>
            <PriceCard
              showCountdown={true}
              product={minangProduct || undefined}
